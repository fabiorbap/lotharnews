...
fun ArticlesList(
    ...
    LazyColumn(
        ...
        items(articles.orEmpty(), key = { item -> item.url ?: UUID.randomUUID() }) {
            CardWithImageAndDescription(
                image = it.urlToImage,
                description = it.title.orEmpty(),
                caption = it.publishedAt?.formatDate().orEmpty(),
                icons = IconToggle(R.drawable.ic_bookmark_saved, R.drawable.ic_bookmark),
                onContentClick = { onCardClick(it.url.orEmpty()) },
                onIconClick = { onCardIconClick(it.url.orEmpty()) },
